const {Discussion, Validate, ValidateMessage} = require('../models/discussion.model');
...
exports.deleteMessageToDiscussion = async (req, res) => {
    ...
    if(!discussion){
        return res.status(400).json({error: "la discussion ou le message n'existe pas"});
    }

    discussion.messages = discussion.messages.filter((message) => _idMessage !== parseInt(message.id_messages))
    await discussion.save()
